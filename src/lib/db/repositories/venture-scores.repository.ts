...
export class VentureScoresRepository {
  ...
  async findByOpportunityId(
    ...
    if (error)
      throw new Error(`[${ENTITY}] findByOpportunityId: ${error.message}`);
    return (data ?? null) as VentureScoreRow | null;
  }
  ...
  async listByRecommendation(
    ...
    if (error)
      throw new Error(`[${ENTITY}] listByRecommendation: ${error.message}`);
    return (data as VentureScoreRow[]) ?? [];
  }
  ...
  async getDashboardStats(): Promise<VentureScoreDashboardStats> {
    ...
    if (error)
      throw new Error(`[${ENTITY}] getDashboardStats: ${error.message}`);
    ...
    let totalScore = 0;
    for (const r of rows) {
      gradeDistribution[r.investment_grade] =
        (gradeDistribution[r.investment_grade] ?? 0) + 1;
      recommendationDistribution[r.recommendation] =
        (recommendationDistribution[r.recommendation] ?? 0) + 1;
    ...
    return {
      ...
      average:
        rows.length > 0
          ? Math.round((totalScore / rows.length) * 100) / 100
          : 0,
      gradeDistribution,
      recommendationDistribution,
      topByROI:
        rows.length > 0 ? Math.max(...rows.map((r) => Number(r.roi_score))) : 0,
      topByConfidence:
        rows.length > 0
          ? Math.max(...rows.map((r) => Number(r.confidence_score)))
          : 0,
      highestRisk:
        rows.length > 0
          ? Math.max(...rows.map((r) => Number(r.risk_score)))
          : 0,
      lowestRisk:
        rows.length > 0
          ? Math.min(...rows.map((r) => Number(r.risk_score)))
          : 0,
    };
  }
