...
export class InvestmentCommitteesRepository {
  ...
  static async create(): Promise<InvestmentCommitteesRepository> {
    ...
    return new InvestmentCommitteesRepository(await createClient());
  }
  ...
  async listSplitVotes(limit = 10): Promise<InvestmentCommitteeRow[]> {
    // Split votes = majority_vote != minority_vote (meaning disagreement)
    const { data, error } = await this.client
      .from(ENTITY)
      .select()
  ...
  async getAgentStats(): Promise<{
    ...
    if (error) {
      // Fallback if votes table not yet ready
    ...
    // Agreement rate: how often all 5 agents agree
